...
{
	relative_label.clear();                                      //清空relative_label
	for (auto it1 = label.begin(); it1 != label.end();)
	{
		for (auto it2 = it1; it2 != label.end();)
		{
			...
			if (score == -1.0f)                                              //如果词典里没有，直接跳出内循环
			{
				++it1;
				break;
			}
			if (score > 0.8)                               //如果相似度超过0.8,标签合并
			{
				int num1= it1->second;
				int num2 = it2 ->second;
				if (num1 >= num2)
...
void up_to_down(vector<pair<string,int>>&a, int i,int &K)
{
	...
	else
	{
		if (t2 > K)
			...
			pos = a[t1].second > a[t2].second ? t2 : t1;         //比较左右孩子节点大小，取较小的那个
...
void create_heap(vector<pair<string, int>>&a, int &K)               //创建堆时从下面开始
{
	int pos = K / 2;
	for (int i = pos; i >= 1; --i)
	{
...
void cilin::sort_label(unordered_map<string, int>&label)
{
	...
	//int total_num = 0;
	//for (auto it = label.begin(); it != label.end();++it)              //总计总的标签数
	//{
	//	total_num += it->second;
	//}
	//int i = 0;
	...
	//}
	//swap(final_label, temp_label);                                  //得到最终的标签,可以重复使用
	int total_num = 0;
	for (auto it = label.begin(); it != label.end();++it)              //总计总的标签数
	{
		total_num += it->second;
	}
	bool flag = false;                                              //建立完成小根堆的标志,统计前5的标签
	...
	{ 
		if (frequency[i].second * 10 > total_num)                      //a*10>total_num的才算进去
